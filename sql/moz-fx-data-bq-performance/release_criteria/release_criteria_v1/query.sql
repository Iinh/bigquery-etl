...
subtests AS (
  ...
  SELECT
    DISTINCT * EXCEPT (rx, rx2, rx3, rx4),
...
WINDOW
  ...
  one_week_window AS (
    moving_average_window RANGE
    BETWEEN 13 PRECEDING
    AND 7 PRECEDING
  ),
  two_week_window AS (
    moving_average_window RANGE
    BETWEEN 20 PRECEDING
    AND 14 PRECEDING
  ),
  three_week_window AS (
    moving_average_window RANGE
    BETWEEN 27 PRECEDING
    AND 21 PRECEDING
  ),
  four_week_window AS (
    moving_average_window RANGE
    BETWEEN 34 PRECEDING
    AND 28 PRECEDING
  )
ORDER BY
